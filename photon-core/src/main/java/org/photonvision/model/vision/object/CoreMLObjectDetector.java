...
public class CoreMLObjectDetector implements ObjectDetector {
    private static final Logger logger = new Logger(CoreMLDetectorJNI.class, LogGroup.General);
    ...
    /**
     * Creates a new CoreMLObjectDetector from the given model.
     *
    ...
    public CoreMLObjectDetector(CoreMLModel model, Size inputSize) {
        ...
        objPointer =
                CoreMLJNI.create(
                        ...
                        CoreMLJNI.CoreMask.ALL.ordinal());
        if (objPointer <= 0) {
            throw new RuntimeException("Failed to create detector from path " + modelPathStr);
        }

        logger.debug(
                "Created detector for model "
                        + model.getName()
                        + " (type: "
                        + (model.isPackage ? "package" : "file")
                        + ")");
    ...
    /**
     * Detects objects in the given input image using the CoreMLDetector.
     *
    ...
    public List<NeuralNetworkPipeResult> detect(Mat in, double nmsThresh, double boxThresh) {
        ...
        var results =
                CoreMLJNI.detect(objPointer, letterboxed.getNativeObjAddr(), nmsThresh, boxThresh);
    ...
    public void release() {
        ...
        if (released.compareAndSet(false, true)) {
            if (objPointer <= 0) {
                logger.error(
                        "Detector is not initialized, and so it can't be released! Model: " + model.getName());
                return;
            }
